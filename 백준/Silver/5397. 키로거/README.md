...
### 오답노트
 ...
 <p>1.새로운 테스트 케이스를 받을 때 unused 등의 함수는 while문 안에서 처음 값으로 초기화해줘야 한다.</p>
 <p>2. unused 함수는 전역 변수로 선언해야 한다.</p>
 <p>3. cursor의 시작 위치를 문자열 S 맨 뒤로 보내서 오류가 났다. 받은 문자열은 명령어 역할, 결과는 빈 문자열에 insert하는 상황이니 cursor의 시작값은 0.</p>

### 최적화
 <p>연결 리스트에서는 나머지 함수에 fill을 쓸 필요 없이, unused = 1과 nxt[0] = -1만 해주면 이전 데이터가 배열에 남아있더라도 새로운 데이터가 그 위를 덮어쓰기 때문에 논리적으로 완벽하게 초기화 된다.</p>
