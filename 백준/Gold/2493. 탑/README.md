...
### 오답노트
 ...
 <p>1. 각 반복문마다 스택을 새로 만들면 O(N^2)의 시간복잡도로 시간초과에 걸린다. O(N)으로 해결하려면 '모노톤 스택(Monotonic Stack)'의 기법이 필요하다.</p>
 <p>2. 인덱스 오류: cout << j+1 부분에서 j는 팝(pop)하는 횟수일 뿐, 실제 탑의 번호가 아니다. pop의 순서와 탑의 번호의 상관관계가 없기 때문에 탑의 번호를 사용하기 위해선 스택에 pair을 사용하여야 한다.</p>
